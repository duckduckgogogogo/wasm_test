...
{
   T*                                  _d;
   typename matrix_type::size_type     _j, _rows, _ld;
#ifndef NDEBUG
   typename matrix_type::size_type     _cols;
#endif
...
public:
   ...
   INLINE segment_type() : _d(NULL), _j(0), _rows(0), _ld(0)
   {
#ifndef NDEBUG
      _cols = 0;
#endif
   ...
   INLINE segment_type (matrix_type& A, size_type j)
      : _d(A.data()+j*A.ld()), _j(j), _rows(A.rows()), _ld(A.ld())   { assert(j <= (_cols=A.cols())); }
   ...
   INLINE iterator end()                                    { return _d + _rows; }
   INLINE const_iterator end() const                        { return _d + _rows; }
   INLINE const_iterator cend() const                       { return _d + _rows; }
   INLINE size_type size() const                            { return _rows; }
   INLINE bool empty() const                                { return _rows == 0; }
   INLINE value_type front() const                          { assert(_rows > 0); return *_d; }
   INLINE reference front()                                 { assert(_rows > 0); return *_d; }
   INLINE value_type back() const                           { assert(_rows > 0); return *(_d+_rows-1); }
   INLINE reference back()                                  { assert(_rows > 0); return *(_d+_rows-1); }
   INLINE value_type operator[] (size_type i) const         { assert(_rows > i); return *(_d+i); }
   INLINE reference operator[] (size_type i)                { assert(_rows > i); return *(_d+i); }
...
public:
   ...
   INLINE const_segment_type() : _d(NULL), _j(0), _rows(0), _ld(0)
   {
#ifndef NDEBUG
      _cols = 0;
#endif
...
/**
   ...
   */
bool 
reserve (size_type m, size_type n) 
{ 
   const size_type   m0 = _rows;
   const size_type   n0 = _cols;
   bool              ok(true);

   if ((m > m0) || (n > n0))
      ok = this->resize(m, n) && this->resize(m0, n0);
...
{ 
   const size_type      m0 = this->rows();
   const size_type      n0 = this->cols();
