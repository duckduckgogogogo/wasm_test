...
/**
   ...
   \param[out]    IDs         The IDs of the columns of \p M1 in \p M0.
                              Column `j` in \p M1 is at column `IDs[j]` in \p M0.
   \param[out]    inv_IDs     The reciprocal vector of \p IDs.
                              Column `j` in \p M0 is at column `inv_IDs[j]` in \p M1.
   ...
   \note          In case of duplicate vectors `v0` in \p M0, the matching index in \p IDs, if any, 
                  is the lowest index of `v0` in \p V0.
   \note          In case of duplicate vectors `v1` in \p V1, the matching index in \p inv_IDs, if any, 
                  is the lowest index of `v1` in \p V1.
   */
CM2MESHTOOLS_API int 
