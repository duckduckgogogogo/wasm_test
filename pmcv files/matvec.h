...
/**
   ...
   \pre     If the matrix is not empty upon entry, 
            the size of vector \p V must be equal to the number of rows in the matrix.
            If the matrix is empty upon entry, the matrix is resized to `V.size()` rows and one column.
   */
...
INLINE void
__mult_omp (T scale, BlockIterator A_0, int nbr_blocks, 
            ...
            IterX x, IterY y, int nthreads) 
{
   ...
   #pragma omp parallel for private(A_k) schedule(static) num_threads(nthreads)
   for (int k = 0; k < nbr_blocks; ++k)      // Parallel loop over in-core blocks.
   {
      A_k = A_0;  A_k += k;
      const size_t         i0    = A_k->first_index();
      const size_t         n     = A_k->dim();
      
      matvec::__mult(scale, *A_k, 
                     cm2::non_symmetric_tag(), ur_orientation_tag(), block_blocking_tag(),
                     x, y, /*nthreads=>*/ 1);
                     
      if (A_k->is_unit_diagonal()) 
         vecvec::axpy_n(scale, x+i0, y+i0, n);
      else
         vecvec::axypz_n(scale, A_k->diagonal_begin(), x+i0, y+i0, n);
   }
}
            ...
            IterX x, IterY y, int nthreads) 
{
   ...
   for (int k = 0; k < nbr_blocks; ++k, ++A_0)      
   {
      const size_t         i0    = A_0->first_index();
      const size_t         n     = A_0->dim();
      
      matvec::__mult(scale, *A_0, 
                     cm2::non_symmetric_tag(), ll_orientation_tag(), block_blocking_tag(),
                     x, y, nthreads);        // Parallel loop inside each block.
                     
      if (A_0->is_unit_diagonal()) 
         vecvec::axpy_n(scale, x+i0, y+i0, n);
      else
         vecvec::axypz_n(scale, A_0->diagonal_begin(), x+i0, y+i0, n);
   }
}
            ...
            IterX x, IterY y, int nthreads) 
{
   ...
   matvec::__mult_omp(scale, A_0, nbr_blocks, 
        ...
        IterX x, IterY y, int nthreads)
{
   ...
   typename Matrix::const_block_iterator     A_0  = A.blocks_begin();
   size_t            nb, se;
   
   for (size_t B0 = 0; B0 < NB; B0 += nb, A_0 += nb)
   {
      (void)A_0->data();                         // Load A_0 and following blocks.
      nb = A.incore_blocks();
      assert(nb <= NB - B0);
      se = A.incore_entries();
      ...
      if (parallel_cond)
         __mult_omp(scale, A_0, nb, 
                    MatrixShape(), typename Matrix::block_type::diagonal_storage_tag(),
                    x, y, nthreads);
      else
#endif
      {
         typename Matrix::const_block_iterator     A_k(A_0);
         for (size_t k = 0; k < NB; ++k, ++A_k)
            matvec::__mult(scale, *A_k, x, y, nthreads);
      }
   }
}
...
template <class T, class BlockIterator, class IterX, class IterY>
INLINE void
__transpose_mult_omp (T scale, BlockIterator A_0, int nbr_blocks, 
                      cm2::non_symmetric_tag, cm2::separate_diagonal_tag /*BlockDiagonalTag*/,
                      IterX x, IterY y, int nthreads) 
{
   typedef typename BlockIterator::value_type               block_type;
    ...
    IterX x, IterY y, int nthreads) 
{
   ...
   for (size_t B0 = 0; B0 < NB; B0 += nb, A_0 += nb)
   {
      ...
      const bool  parallel_cond = (nthreads > 1) && (nb > 1) && (se > CM2_OMP_MIN_LOOP);
      if (parallel_cond)
         __transpose_mult_omp(scale, A_0, nb, 
                              MatrixShape(), typename Matrix::block_type::diagonal_storage_tag(),
                              x, y, nthreads);
      else
#endif
      {
         typename Matrix::const_block_iterator     A_k(A_0);
         for (size_t k = 0; k < NB; ++k, ++A_k)
            matvec::__transpose_mult(scale, *A_k, x, y, nthreads);
      }
   }
}
...
__Vt_A_V (const Matrix& A, cm2::blocked_tag, IterV v, int nthreads) 
{
   ...
   for (size_t B0 = 0; B0 < NB; B0 += nb, A_0 += nb)
   {
      ...
      #pragma omp parallel for if(parallel_cond) reduction(+:s) private(A_k) schedule(static) num_threads(nthreads)
      for (int k = 0; k < int(nb); ++k)         // Parallel loop over in-core blocks.
      {
         A_k = A_0;  A_k += k;
         s += matvec::__Vt_A_V(*A_k, block_diagonal_storage_tag(), v, 
