...
/** 
   ...
   If i < size-1, the linear interpolation is coeff*range[i] + (1.-coeff)*range[i+1] (coeff = (domain[hi] - d) / (domain[hi] - domain[lo])).

   If i == size-1, the linear interpolation is range[i] (coeff == 1.0).

   If size == 0 return pair(CM2_NONE, 0.0)
...
interpolation (const typename VectorD::value_type& d, const VectorD& domain) 
{
   ...
   if (domain.size() == 0) 
      return std::pair<unsigned, double>(CM2_NONE, 0.0);
   if (domain.size() == 1) 
      return std::pair<unsigned, double>(0, 1.0);

   p = std::lower_bound(ALL(domain), d);      // lowest position of domains >= d.

   if (p == domain.end())        
      return std::pair<unsigned, double>(domain.size()-1, 1.0);
   if (p == domain.begin()) 
      return std::pair<unsigned, double>(0, 1.0);

   hi = p - domain.begin();
   lo = hi - 1;
   coeff = (double)(domain[hi] - d) / (double)(domain[hi] - domain[lo]);    
   return std::pair<unsigned, double>(lo, coeff);
}
...
interpolation (const typename VectorD::value_type& d, const VectorD& domain, const VectorR& range) 
{
   ...
   const unsigned                      i     = P.first;
   const double                        ci    = P.second;
   ...
   return v2 + ci * (v1 - v2);         // linear interpolation.
}
               ...
               const VectorD1& D1, const VectorD2& D2, const MatrixR& R) 
{
   ...
   const unsigned                      i     = Pi.first;
   const unsigned                      j     = Pj.first;
   const double                        ci    = Pi.second;
   const double                        cj    = Pj.second;
