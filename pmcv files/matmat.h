...
mult (T a, const MatA& A, const MatB& B, MatC& C,
      size_t cache_size = CM2_DEFAULT_LLC_SIZE, int nthreads = 1u);
    ...
    size_t cache_size, int nthreads) 
{
   ...
   // Compute the number of columns fitting in cache.
   {
      ...
      if (cache_size < Ci*a_ + Cj*(b_+c_))      // Don't fit in cache.
      {
...
#ifdef _OPENMP
      if ((nthreads > 1) && (cols > 1) && (A.entries()*cols > CM2_OMP_MIN_LOOP))    // Parallel version.
      {
    ...
    size_t /*cache_size*/, int nthreads) 
{
   ...
   for (size_t j0 = 0; j0 < COLS; j0 += cols, b0 += cols, c0 += cols) 
   {
      ...
      for (A0 = A.blocks_begin(); A0 < A.blocks_end(); A0 += blocks) 
      {
         ...
         {
            typename MatA::const_block_iterator    A_k(A0);
            typename MatB::const_segment_iterator  b(b0);  b += j; 
            typename MatC::segment_iterator        c(c0);  c += j; 
            for (size_t k = 0; k < blocks; ++k, ++A_k) 
               matvec::__mult(a, *A_k, b.begin(), c.begin(), /*nthreads=>*/ 1);
         }
      }
   } 
}
    ...
    size_t cache_size, int nthreads) 
{
   ...
   // Compute the number of columns fitting in cache.
   {
      ...
      const size_t   b_  = B.rows() * sizeof(TB);
      const size_t   c_  = C.dim()  * sizeof(TC);
      if (cache_size < Ci*(a_+b_+c_))              // Don't fit in cache.
      {
   ...
   for (size_t j0 = 0; j0 < COLS; j0 += cols, b0 += cols, c0 += cols) 
   {
      ...
      for (A0 = A.blocks_begin(); A0 < A.blocks_end(); A0 += blocks) 
      {
         ...
         for (int j = 0; j < cols; ++j)
         {
            ...
            typename MatC::segment_iterator        c(c0); c += j; 
            for (size_t k = 0; k < blocks; ++k, ++A_k) 
               matvec::__transpose_mult(a, *A_k, b.begin(), c.begin(), /*nthreads=>*/ 1);
         }
      }
   } 
}
...
void operator() (T a, const MatA& A, const MatB& B, MatC& C,
                 size_t cache_size = CM2_DEFAULT_LLC_SIZE, int nthreads = 1u) const
{
   typedef typename MatB::value_type   TB;
