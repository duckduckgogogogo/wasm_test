/**
   ...
   $Rev: 365 $
   $Date: 2008-08-08 16:07:14 +0200 (ven., 08 août 2008) $ 
   */
...
/**
   ...
   \note       Only isotropic target sizes are taken into account. \n
               The first component of mesher2D.settings.target_metric is used to define
               the target size (i.e. mesh size inside the surface).
   \note       There is no mesh gradation control
               when \p mesher2D.settings.target_metric(0,0) <= 0 or when \p mesher2D.settings.max_gradation <= 0.
   \note       With a mesh gradation control (\p mesher2D.settings.target_metric(0,0) > 0 and 
               when \p mesher2D.settings.max_gradation > 0), the input metrics on the background mesh (if any) are discarded.
   \note       Upon exit, the \p data3D.metrics is a 3-D anisotropic metric arrays (6-row matrix) defined only
               at the hard nodes.
   \note       Edge qualities are not computed, even when \p mesher2D.settings.compute_Qh_flag = true.
...
//meshtools::medit_tensor("bgm.bb", aux_data2D.metrics);
   ...
   if ((ret >= 0) && !HAS_BGM)
   {
      ...
      // Uniform metrics3D on new inside nodes (N0 -> N1).  
      if (TARGET_H_OK)                  
      {
         DoubleSym3     M3;
      ...
      else
      {
         ...
         if ((ret >= 0) && !aux_data2D.connectM.empty())
            ret = meshtools2d::interpolate_metrics(aux_data2D.pos, aux_data2D.connectM, 
                                                   ...
                                                   metrics3D, /*invalid_metrics_only=>*/ true);
      }
   ...
   // Adapt the 3D metrics to respect max_gradation.
   if (ret >= 0)
      ret = __bound_metric3D_gradations(S, data2D, 
                                        N0, N1, hnodes, 0, mesher2D.settings.max_gradation,
                                        pos3D, metrics3D);

   // Bound the 3-D metrics with curvature metrics.
   if (ret >= 0)
   ...
   // Adapt the 3D metrics to respect max_gradation (only shrinkage).
   if (ret >= 0)
      ret = __bound_metric3D_gradations(S, data2D, 
   ...
   if (!data3D.pos.resize(3, NP + (N1-N0), 0.0))
      return data3D.error_code = AnisoMesher::data_type::CM2_SYSTEM_MEMORY_ERROR;
...
#if (_MSC_VER >= 1400) 
            ::strncpy_s(data3D.msg1, 256, "DEGENERATED ELEMENT", 256);
#else
            ::strncpy(data3D.msg1, "DEGENERATED ELEMENT", 256);
#endif
         }
...
#if (_MSC_VER >= 1400) 
            ::strncpy_s(data3D.msg1, 256, "SHAPE QUALITY WARNING", 256);
#else
            ::strncpy(data3D.msg1, "SHAPE QUALITY WARNING", 256);
#endif
         }
      }
