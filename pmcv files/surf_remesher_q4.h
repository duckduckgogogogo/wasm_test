#ifndef __SURF_REMESHER_Q4_H__
#define __SURF_REMESHER_Q4_H__

/**
   \file       surf_remesher_q4.h
   \brief      Definition of the 3d-surface remesher Q4.
...
/**
   3-D surface quad-dominant remesher.
   ...
   \note       Thread safe.
   \see        surf_remesh_t3::mesher, hexamesh::mesher.

   \image html pulley_i.png  "Initial 3-D surface (STL mesh)"
   \image html pulley_rq.png "Remeshed 3-D surface (all-quad mode)"
   \image html pulley_rqt.png "Remeshed 3-D surface (mixed quad-dominant mode, 92% quads, 8% triangles)"
   */
class CM2SURFREMESHQ4_API mesher
{
public:

   /**
      This generator is for all-quad or mixed triangle/quadrangles meshes.
      */
   enum
   {
   ...
   settings_type()
      : min_h(0.0), max_h(DBL_MAX), target_h(0.), max_gradation_1d(0.5), max_gradation_2d(0.5), max_chordal_error(-0.02), 
        ...
        initial_cleanup_flag(true), notch_closing_flag(true), closed_manifold_flag(false), 
        remesh_flag(true), all_quad_flag(false), force_even_flag(false),
        ...
        structured_flag(true), nodes_limit(UINT_MAX), optim_level(3), shape_quality_weight(0.6), 
        quadrangle_weight(0.70), min_Q4_angle_quality(0.20), length_upper_bound(1.414), 
        min_NE_for_loops(6), max_NE_for_loops(UINT_MAX),
   ...
   /**
      Resets to the \link cm2::surf_remesh_q4::mesher::settings_type::settings_type() default values \endlink.
      */
   void
   reset();
   ...
   /**
      ...
      \note    All-quad meshing (\ref all_quad_flag = true) usually requires an even number 
               of edges along each skeleton line.\n
               When all-quad meshing is requested, the remesher may force this flag and
               proceed to even-remeshing of skeleton lines.
   ...
   /**
      ...
      \note    With \ref remesh_flag = false, shell-remeshing can be used to 
               convert triangles into quads without changing the nodes. 
   ...
   /**
      ...
      Default = true (structured mesh when possible).
      */
   ...
   /**
      The data type for surf_remesh_q4::mesher.

      This structure is used to gather all the input and output data of the function 
      surf_remesh_q4::mesher::run.
      */
   struct CM2SURFREMESHQ4_API data_type
   {
   public:
   ...
   /**
      ...
      \param[out]    pos_        A view to the \ref pos matrix.
      \param[out]    connectM_   A view to the \ref connectM matrix. This matrix has dimensions 4 x nefs.\n
   ...
   /**
      ...
      Example:
      \code
         surf_remesh_q4::mesher              my_remesher;
         surf_remesh_q4::mesher::data_type   my_data(pos, connect_in);     // Initial triangle mesh in pos and connect_in.
   ...
   /**
      ...
      This matrix stores the connectivity of the triangle mesh upon entry (dimension 3 x \ref nefs_in)
      and the connectivity of the quad-dominant or all-quad mesh upon exit (dimension always 4 X \ref nefs).

      Upon exit, the leading part of this matrix (from col 0 to col \ref nefs_Q4 - 1)
      is the connectivity of the quadrangle elements. \n
      The tailing part (from col \ref nefs_Q4 to col \ref nefs - 1) is the connectivity of the triangle elements.\n
      The fourth node ID in this triangle part is always `CM2_NONE` (i.e. `unsigned(-1)`).

      \note    The input mesh must be a _pure-triangle_ mesh.
               You can use meshtools2d::split_Q4_into_T3 to transform all-quad or mixed quad-triangle meshes
               into pure-triangle meshes.
      \note    To keep only the nodes of the final mesh, use meshtools::simplify.
   ...
   /**
      ...
      nefs_Q4 = nefs with an all-quad mesh.
   ...
   /**
      ...
      nefs_T3 = 0 with a all-quad mesh.
   ...
   /**
      ...
      \note    area = area_Q4 + area_T3
      \note    The area of warped quads is approximative.
...
/**
   ...
   See \link cm2::surf_remesh_q4::mesher::settings_type::settings_type() default values \endlink.
   */
mesher();
...
/**
   ...
   \see  surf_remesh_q4::registration.
   */
static bool
authorized();
