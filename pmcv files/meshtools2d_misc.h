...
/**
   ...
   \note       In 2-D (when \p pos.rows() == 2), the orientation of the each element is positive when
               oriented counter-clockwise (CCW), and negative when oriented clockwise (CW).
...
/**
   Compute the nodal averaged normals along a 3-D surface.
...
/**
   ...
   \param[in]     R              Radius of the rings. \n
                                 Only pairs of elements in the ring of node `Ni` are considered
                                 for the computation of the curvatures at `Ni`. \n
                                 The ring is defined by the elements directly connected to `Ni` (ring-1)
                                 plus all elements with at least one node within the sphere of radius \p R centred at `Ni`.
   \param[in]     angle_max      Only contributions of pairs of elements with angle smaller than this threshold
                                 (expressed in radian) are used in the computation of the curvatures. \n
   \param[in]     max_nefs_in_rings    Limit the rings to the \p max_nefs_in_rings "closest" elements.
   \param[in]     boundaries     Optional matrix (2xNE) to define additional "angulous" edges (in addition to those with
   ...
   \param[out]    curvatures     The approximated nodal curvatures as a 8xNODS matrix. \n
                                 ...
                                 For each node `i`:
                                    ...
                                    -  `curvatures(3,i)` = \f$ K_0(i) \f$.
                                    -  `curvatures(4,i)` = \f$ K_1(i) \f$.
                                    .
...
principal_curvatures
   (const DoubleMat& pos, const UIntMat& connectM, const UIntMat& neighbors, 
    double R, double angle_max, unsigned max_nefs_in_rings, const UIntMat& boundaries,
