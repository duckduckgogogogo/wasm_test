...
/**
   ...
   \warning       To recompute the ancestors from scratch, clear vector \p ancestor before entry.
   \warning       A mixed mesh (for instance `CM2_FACET3` and `CM2_FACEQ4`) contains node 
                  indices with value `CM2_NONE`. In this case, the \p ancestors vector must not
                  be empty upon entry (because it cannot be resized inside the function).
   */
...
/**
   ...
   \pre           \p connect has the right number of rows (defined by \p FE_type) or empty.
...
/**
   ...
   \param[in]     neighbors      The neighbors matrix defined by: 
                                 ...
                                 This matrix can be computed with meshtools::get_neighbors (above) or by a CM2 mesher.
   \param[in]     FE_type        The type of the elements (\ref fe_geo_page). \n
                                 Only linear elements are allowed: cm2::CM2_EDGE2, cm2::CM2_FACET3, cm2::CM2_FACEQ4,
                                 cm2::CM2_FACE_MIX, cm2::CM2_TETRA4,cm2::CM2_PYRAMID5, cm2::CM2_WEDGE6, cm2::CM2_HEXA8,
                                 cm2::CM2_SOLID_MIX.
   ...
   \note          The mesh can be a mixed type (cm2::CM2_FACE_MIX or cm2::CM2_SOLID_MIX).
   \note          The meshtools::get_mesh_boundaries and the meshtools::get_mesh_bounding_elements give the 
                  same boundaries, in same order. \n
                  The first function gives boundaries in _explicit_ form (connectivity matrix of the boundaries)
                  whereas the second function gives boundaries in _implicit_ form (element ID and local boundary ID).
   \note          The \p neighbors matrix can be computed by the meshtools::get_neighbors function.
   \note          Time and memory complexity: linear = O(\p connect.entries()).
...
/**
   ...
   \note          The meshtools::get_mesh_boundaries and the meshtools::get_mesh_bounding_elements give the 
                  same boundaries, in same order.\n
                  The first function gives boundaries in _explicit_ form (connectivity matrix of the 
                  boundaries) whereas this function gives boundaries in _implicit_ form (element IDs 
                  and local boundary IDs).
   \note          The \p neighbors matrix can be computed by the meshtools::get_neighbors function.
   \note          Time and memory complexity: linear = O(\p connect.entries()).
...
/**
   ...
   \pre           \p connect has the right number of rows (= number of nodes defined 
                  by \p FE_type) or empty.
   \pre           Upon entry \p boundaries is empty or has the right number of rows 
                  (= number of nodes of boundary type).
...
/**
   ...
   \param[out]    adjncy         The CSR connections. \n
                                 Elements connected to node `i` are between `adjncy[xadj[i]]` and 
                                 `adjncy[xadj[i+1]]` (past-the-end).
...
/**
   ...
   \param[out]    xadj           The CSR indices (positions in \p adjncy where node connections are located).
   \param[out]    adjncy         The CSR connections. \n
                                 Nodes connected to node `i` (except node `i` itself) are between 
                                 `adjncy[xadj[i]]` and `adjncy[xadj[i+1]]` (past-the-end).
...
/**
   ...
   \param[in]     ancestors      The ancestors vector defined by:
                                 ...
                                 This array can be computed with meshtools::get_ancestors (above) or by a CM2 mesher.
   \param[in]     FE_type        The type of the elements (\ref fe_geo_page).
